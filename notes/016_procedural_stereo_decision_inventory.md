...
## 2026-05-18 Completion/Frontier Identity Audit

This update reflects the current state after the support-boundary carrier
diagnostics, visible-marker-basis diagnostics, and row-witness-gated deferred
marker broadening slices.

There are now two different identity questions:

- public decoder/frontier identity is intentionally conservative: merged
  decoder cache keys still serialize the whole frontier, and Rust state
  ordering still compares the procedural vectors in
  `cmp_stereo_state_structure`;
- internal completion memoization is narrower: `StereoCompletionKey` includes
  structural traversal state plus procedural stereo vectors, but excludes the
  human-readable `prefix`, `directional_marker_traces`, and
  `marker_event_traces`.

That split is acceptable only if every support-shaping marker fact used during
completion is either derivable from the included structural/procedural vectors
or is already represented by an included fact vector. This is the main audit
frontier for future cleanup.

### Current true boundary facts

These walker fields still feed the support boundary directly or through a
small named adapter, so they must remain in completion identity until their
replacement support-state representation exists:

| Walker state field | Current boundary role | Why it remains in identity |
| --- | --- | --- |
| `stereo_selected_neighbors` | raw carrier-selection facts | unresolved/raw carrier choices seed `support_boundary_facts_from_walker_state`; joined support-boundary resolution is now primary, but the raw vector is still the stored input |
| `deferred_carrier_choice_constraints` | `CarrierSelectionBlocked` facts | this is already a typed support-boundary fact source and affects carrier availability |
| `committed_component_token_flips` | known token-flip facts | known committed flips override observations and constrain token phase |
| `stereo_token_basis_facts` | isolated component writer-basis facts | currently the only persisted source for isolated slash/backslash basis during rendering |
| `stereo_component_phases` | token-observation input | feeds typed token observations and RDKit token-flip adjustment logic |
| `stereo_component_begin_atoms` | token-observation input | distinguishes begin-side/root policy in token observation construction |
| `stereo_first_emitted_candidates` | RDKit traversal observation input | feeds first-emitted-candidate and adjacent two-candidate token-flip adjustment |
| `stereo_selected_orientations` | RDKit traversal/token observation input | still participates in orientation-sensitive token observations |
| structural traversal state | graph traversal identity | `visited`, `pending`, `free_labels`, `next_label`, and `action_stack` determine future structural emissions |

The right long-term target is not to drop these facts from identity; it is to
replace the procedural vectors with one explicit support-state identity that
already contains their semantic/writer-policy consequences.

### Shadow-only diagnostics

These values should not become new runtime truth:

- `legacy_field_resolved_selected_neighbors` in output diagnostics;
- `resolved_selected_neighbors_from_assignment_state` in output diagnostics;
- the `joined_support_boundary_*` comparison booleans in `shadow_debug`;
- `legacy_topology_guard_applies` in visible-marker-basis diagnostics.

They are useful while reviewing equivalence and regressions, but any future
runtime decision should consume the named support-boundary facts or row/model
queries instead.

### Fields that must stay for now

Some fields are conceptually temporary but not yet removable:

- `committed_component_token_flips` still has to stay until deferred token
  commits are represented as constraint-state transitions rather than walker
  vector mutation plus a post-commit assertion.
- `stereo_component_phases`, `stereo_component_begin_atoms`,
  `stereo_first_emitted_candidates`, and `stereo_selected_orientations` still
  have to stay until token observation facts are produced as first-class
  boundary events rather than reconstructed from walker vectors.
- `deferred_carrier_choice_constraints` should stay because it is already a
  typed fact source; cleanup should move its production point, not erase it.
- `stereo_token_basis_facts` should stay until isolated component basis
  selection is generated by the same boundary that owns token observations.
- visible-marker compatibility still needs the residual topology gate retained
  by `6d9fafe`; the row witness is now explicit, but task `Visible marker
  basis 04b` must replace the remaining topology condition before
  `runtime_component_has_cross_component_carrier_edge` can be retired.

### Marker traces and completion cache risk

`marker_event_traces` and `directional_marker_traces` are support-relevant, but
they are not included in `StereoCompletionKey`. Today this is only safe if
states that collide in the completion key cannot differ in row-relevant marker
history. Before promoting marker obligations further, add one of these:

- an assertion/test proving marker event facts are determined by the existing
  completion key for all merged states; or
- the minimal marker-event identity needed by
  `support_boundary_facts_from_walker_state` into `StereoCompletionKey`.

Do not remove marker traces as "diagnostic only" while marker-row survivor
state and visible-marker-basis acceptance still consume them.
