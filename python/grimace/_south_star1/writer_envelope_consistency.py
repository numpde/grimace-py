...
def verify_writer_support_image_envelope_consistency(envelope: object, *, budget: WriterEnvelopeWorkBudget | None=None) -> WriterEnvelopeConsistencyVerification:
    try:
        ...
        return WriterEnvelopeConsistencyVerification(accepted=True, schema_name=str(envelope['schema_name']), schema_version=int(envelope['schema_version']), support_count=int(envelope['distinct_count']), witness_count=int(envelope['witness_count']))
    except WriterEnvelopeWorkExceeded as exc:
        return WriterEnvelopeConsistencyVerification(accepted=False, schema_name=str(envelope.get('schema_name', 'unknown')) if isinstance(envelope, Mapping) else 'unknown', schema_version=envelope.get('schema_version') if isinstance(envelope, Mapping) and isinstance(envelope.get('schema_version'), int) else None, reason=writer_envelope_work_reason(exc))
    except _ConsistencyViolation as exc:
        return WriterEnvelopeConsistencyVerification(accepted=False, schema_name=str(envelope.get('schema_name', 'unknown')) if isinstance(envelope, Mapping) else 'unknown', schema_version=envelope.get('schema_version') if isinstance(envelope, Mapping) and isinstance(envelope.get('schema_version'), int) else None, reason=str(exc))
    except (AssertionError, KeyError, TypeError, ValueError) as exc:
        return WriterEnvelopeConsistencyVerification(accepted=False, schema_name=str(envelope.get('schema_name', 'unknown')) if isinstance(envelope, Mapping) else 'unknown', schema_version=envelope.get('schema_version') if isinstance(envelope, Mapping) and isinstance(envelope.get('schema_version'), int) else None, reason=f'malformed_envelope:{type(exc).__name__}')
_ADVANCE_FIELDS = frozenset(('schema_name', 'schema_version', 'prepared_identity', 'source_snapshot', 'emitted_text', 'outcome_kind', 'frontier_product_kind', 'advance_certificate'))
_REPLAY_FIELDS = frozenset(('schema_name', 'schema_version', 'prepared_identity', 'source_snapshot', 'emitted_texts', 'outcome_kind', 'consumed_emitted_texts', 'remaining_emitted_texts', 'step_advance_envelopes', 'current_snapshot', 'replay_certificate', 'failed_advance_envelope'))
_PREFIX_FIELDS = frozenset(('schema_name', 'schema_version', 'prepared_identity', 'source_snapshot', 'emitted_texts', 'include_counts', 'read_kind', 'replay_envelope', 'final_snapshot', 'final_frontier_product_kind', 'final_frontier_product', 'prefix_read_certificate', 'public_frontier', 'support_count', 'completion_count', 'failure'))
_COUNT_FIELDS = frozenset(('schema_name', 'schema_version', 'prepared_identity', 'source_kind', 'source_snapshot', 'prefix_read_envelope', 'frontier_snapshot', 'frontier_product', 'support_count', 'completion_count', 'count_dag', 'support_count_certificate', 'completion_count_certificate', 'choice_count_certificates', 'terminal_choice_count_certificate', 'coverage'))
_SUPPORT_STRING_FIELDS = frozenset(('schema_name', 'schema_version', 'prepared_identity', 'source_kind', 'source_snapshot', 'prefix_read_envelope', 'count_envelope', 'string', 'emitted_texts', 'replay_envelope', 'final_snapshot', 'terminal_frontier_product', 'terminal_frontier_projection', 'terminal_projection', 'support_string_certificate', 'terminal_support_identities', 'text_projection_chain', 'digest'))
_SUPPORT_IMAGE_FIELDS = frozenset(('schema_name', 'schema_version', 'prepared_identity', 'source_kind', 'source_snapshot', 'prefix_read_envelope', 'count_envelope', 'support_strings', 'support_string_envelopes', 'distinct_count', 'witness_count', 'support_image_certificate', 'enumeration_coverage', 'frontier_product', 'checked_frontier_certificate', 'support_count_certificate', 'witness_count_certificate'))
...
def _validate_support_image_envelope(envelope: object) -> None:
    ...
    _check_common_source(outer=envelope, nested=count, outer_source=source, nested_source=count['frontier_snapshot'], label='count_envelope')
    if envelope['frontier_product'] != count['frontier_product']:
        _fail('frontier_product_identity_mismatch')
    if envelope['checked_frontier_certificate'] != count['frontier_product']['checked_frontier_certificate']:
    ...
    if envelope['witness_count_certificate'] != count['completion_count_certificate']:
    ...
    for index, (string, string_envelope) in enumerate(zip(strings, string_envelopes, strict=True)):
        _validate_support_string_envelope(string_envelope, expected_source_kind=envelope['source_kind'], expected_source=source, expected_prefix=envelope['prefix_read_envelope'], expected_count=count, expected_string=string, index=index)
...
def _validate_count_envelope(envelope: object) -> None:
    ...
    if envelope['frontier_product']['cursor'] != envelope['frontier_snapshot']['cursor']:
        _fail('count_frontier_cursor_mismatch')
    _validate_self_digest(envelope['frontier_product'], 'frontier_product')
    _validate_self_digest(envelope['frontier_product']['checked_frontier_certificate'], 'checked_frontier_certificate')
    try:
    ...
    if envelope['completion_count_certificate'] != nodes[roots['completion_count_root']]:
        _fail('completion_count_root_mismatch')
    if envelope['choice_count_certificates'] != [nodes[node_id] for node_id in roots['choice_count_roots']]:
    ...
    if envelope['support_count_certificate']['support_count'] != envelope['support_count']:
        _fail('support_count_certificate_total_mismatch')
    if envelope['completion_count_certificate']['completion_count'] != envelope['completion_count']:
...
def _validate_count_coverage_node_references(coverage: Mapping[str, object], nodes: Mapping[str, Mapping[str, object]], terminal_root) -> None:
...
def _validate_support_string_envelope(envelope: object, *, expected_source_kind: str, expected_source, expected_prefix, expected_count, expected_string, index: int) -> None:
    ...
    if envelope['support_string_certificate']['terminal_projection_digest'] != terminal['digest']:
        _fail(f'support_string_terminal_projection_digest_mismatch:{index}')
    if envelope['support_string_certificate']['text_projection_chain_digests'] != [step['text_projection']['digest'] for step in envelope['text_projection_chain']]:
...
def _validate_replay_envelope(envelope: object) -> None:
    ...
    assert isinstance(envelope, Mapping)
    if envelope['outcome_kind'] not in ('advanced', 'invalid_emitted_text', 'blocked'):
...
def _validate_prefix_envelope(envelope: object) -> None:
    ...
    assert isinstance(envelope, Mapping)
    if envelope['read_kind'] not in ('readable', 'replay_blocked', 'invalid_emitted_text', 'final_frontier_blocked'):
...
def _validate_text_projection_chain(envelope: Mapping[str, object]) -> None:
    ...
    for index, (chain_step, replay_step) in enumerate(zip(chain, replay_steps, strict=True)):
        ...
        if _text_bucket_key(chain_step['text_projection']) != _text_bucket_key(selected):
...
def _validate_support_image_certificate(envelope: Mapping[str, object]) -> None:
    ...
    if certificate['support_count_certificate_digest'] != envelope['support_count_certificate']['digest']:
        _fail('support_image_certificate_support_digest_mismatch')
    if certificate['witness_count_certificate_digest'] != envelope['witness_count_certificate']['digest']:
        _fail('support_image_certificate_witness_digest_mismatch')
    if certificate['checked_frontier_certificate_digest'] != envelope['checked_frontier_certificate']['digest']:
        _fail('support_image_certificate_checked_digest_mismatch')
    if certificate['enumeration_coverage_digest'] != envelope['enumeration_coverage']['digest']:
        _fail('support_image_certificate_coverage_digest_mismatch')
    expected_string_digests = [item['support_string_certificate']['digest'] for item in envelope['support_string_envelopes']]
    if certificate['string_certificate_digests'] != expected_string_digests:
        _fail('support_image_certificate_string_digest_mismatch')

def _validate_enumeration_coverage(envelope: Mapping[str, object], source) -> None:
    ...
    for bucket in coverage['text_buckets']:
        ...
        for index in indices:
            ...
            first_projection = string_envelope['text_projection_chain'][0]['text_projection']
            if _text_bucket_key(first_projection) != _text_bucket_key(bucket['text_projection']):
                _fail('text_bucket_projection_mismatch')
            if string_envelope['support_string_certificate']['digest'] not in bucket['string_digests']:
                _fail('text_bucket_string_digest_mismatch')
        assigned_indices.extend(indices)
    terminal = coverage['terminal_bucket']
    empty_indices = [index for index, item in enumerate(string_envelopes) if not item['emitted_texts']]
    if empty_indices:
        if terminal is None:
        ...
        if terminal['terminal_support_identities'] != terminal_string['terminal_support_identities']:
...
def _check_consistency_work(envelope: Mapping[str, object], *, budget: WriterEnvelopeWorkBudget) -> None:
    nested_count = _count_nested_envelopes(envelope)
    check_writer_envelope_work(budget=budget, operation='support_image_consistency', metric='nested_envelope_count', actual=nested_count, limit=budget.max_nested_envelopes)
    check_writer_envelope_work(budget=budget, operation='support_image_consistency', metric='consistency_node_count', actual=_count_json_nodes(envelope), limit=budget.max_consistency_nodes)
    strings = _require_list(envelope, 'support_strings')
    string_envelopes = _require_list(envelope, 'support_string_envelopes')
    check_writer_envelope_work(budget=budget, operation='support_image_consistency', metric='support_string_count', actual=len(strings), limit=budget.max_support_strings)
    check_writer_envelope_work(budget=budget, operation='support_image_consistency', metric='support_string_envelope_count', actual=len(string_envelopes), limit=budget.max_support_string_envelopes)
    check_writer_envelope_work(budget=budget, operation='support_image_consistency', metric='total_emitted_text_bytes', actual=sum((len(text.encode('utf-8')) for string_envelope in string_envelopes for text in string_envelope['emitted_texts'])), limit=budget.max_total_emitted_text_bytes)
    coverage = envelope['enumeration_coverage']
    ...
    if terminal is not None and terminal['string_index'] is not None:
        assignment_count += 1
    check_writer_envelope_work(budget=budget, operation='support_image_consistency', metric='coverage_bucket_count', actual=bucket_count, limit=budget.max_coverage_buckets)
    check_writer_envelope_work(budget=budget, operation='support_image_consistency', metric='bucket_assignment_count', actual=assignment_count, limit=budget.max_bucket_assignments)
...
def _check_common_source(*, outer, nested, outer_source, nested_source, label: str) -> None:
...
def _validate_self_digest(envelope: Mapping[str, object], label: str) -> None:
    ...
    expected = _identity_digest({key: value for key, value in envelope.items() if key != 'digest'}, budget=WriterEnvelopeWorkBudget(), operation=f'consistency.{label}.digest')
    if envelope['digest'] != expected:
        _fail(f'{label}_digest_mismatch')

def _require_schema(envelope: object, schema_name: str, schema_version: int, fields: frozenset[str]) -> None:
...
def _text_bucket_key(projection: Mapping[str, object]):
    return (projection['source_cursor']['digest'], projection['emitted_text'], projection['successor_cursor']['digest'], projection['immediate_multiplicity'])
...
__all__ = ('WriterEnvelopeConsistencyVerification', 'verify_writer_support_image_envelope_consistency')
