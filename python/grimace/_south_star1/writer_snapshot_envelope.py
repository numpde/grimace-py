...
def _frontier_projection_envelope(projection, *, budget) -> dict[str, object]:
    envelope = {'cursor': _cursor_envelope(projection.cursor, budget=budget, operation='snapshot_advance.frontier_projection.cursor'), 'text_projection_digests': [_text_projection_digest(item, budget=budget, operation='snapshot_advance.frontier_projection.text_projection') for item in projection.text_choice_projection_certificates], 'text_projection_keys': [_text_projection_key(item, budget=budget) for item in projection.text_choice_projection_certificates], 'terminal_projection_digest': None if projection.terminal_projection_certificate is None else _terminal_projection_digest(projection.terminal_projection_certificate, budget=budget, operation='snapshot_advance.frontier_projection.terminal_projection'), 'branch_count': len(projection.branch_certificates), 'terminal_count': len(projection.terminal_certificates)}
    envelope['digest'] = _identity_digest(_frontier_projection_manifest(envelope), budget=budget, operation='snapshot_advance.frontier_projection')
    return envelope

def _text_projection_envelope(projection, *, budget) -> dict[str, object]:
    envelope = {'source_cursor': _cursor_envelope(projection.source_cursor, budget=budget, operation='snapshot_advance.text_projection.source_cursor'), 'emitted_text': projection.emitted_text, 'successor_cursor': _cursor_envelope(projection.successor_cursor, budget=budget, operation='snapshot_advance.text_projection.successor_cursor'), 'immediate_multiplicity': projection.immediate_multiplicity, 'support_count': projection.support_count, 'completion_count': projection.completion_count, 'projection_key': _text_projection_key(projection, budget=budget), 'branch_certificate_digests': [_branch_certificate_digest(item, budget=budget, operation='snapshot_advance.text_projection.branch_certificate') for item in projection.branch_certificates]}
    envelope['digest'] = _identity_digest(_text_projection_manifest(envelope), budget=budget, operation='snapshot_advance.text_projection')
    return envelope

def _frontier_projection_digest(projection, *, budget, operation: str) -> str:
    envelope = _frontier_projection_envelope(projection, budget=budget)
    return _identity_digest(_frontier_projection_manifest(envelope), budget=budget, operation=operation)

def _text_projection_digest(projection, *, budget, operation: str) -> str:
    envelope = _text_projection_envelope(projection, budget=budget)
    return _identity_digest(_text_projection_manifest(envelope), budget=budget, operation=operation)

def _terminal_projection_digest(projection, *, budget, operation: str) -> str:
    manifest = {'source_cursor_digest': _cursor_envelope(projection.source_cursor, budget=budget, operation=f'{operation}.source_cursor')['digest'], 'finalized_cursor_digest': _cursor_envelope(projection.finalized_cursor, budget=budget, operation=f'{operation}.finalized_cursor')['digest'], 'multiplicity': projection.multiplicity, 'support_count': projection.support_count, 'completion_count': projection.completion_count, 'terminal_count': len(projection.terminal_certificates)}
    return _identity_digest(manifest, budget=budget, operation=operation)

def _branch_certificate_digest(certificate, *, budget, operation: str) -> str:
    manifest = {'source_state_digest': _identity_digest(certificate.source_state, budget=budget, operation=f'{operation}.source_state'), 'successor_state_digest': _identity_digest(certificate.successor_state, budget=budget, operation=f'{operation}.successor_state'), 'emitted_text': certificate.emitted_text, 'parent_weight': certificate.parent_weight, 'branch_ordinal': certificate.branch_ordinal}
    return _identity_digest(manifest, budget=budget, operation=operation)

def _frontier_projection_manifest(envelope: Mapping[str, object]) -> dict[str, object]:
    return {'cursor_digest': envelope['cursor']['digest'], 'text_projection_digests': envelope['text_projection_digests'], 'terminal_projection_digest': envelope['terminal_projection_digest'], 'branch_count': envelope['branch_count'], 'terminal_count': envelope['terminal_count']}

def _text_projection_manifest(envelope: Mapping[str, object]) -> dict[str, object]:
    return {'source_cursor_digest': envelope['source_cursor']['digest'], 'emitted_text': envelope['emitted_text'], 'successor_cursor_digest': envelope['successor_cursor']['digest'], 'immediate_multiplicity': envelope['immediate_multiplicity'], 'support_count': envelope['support_count'], 'completion_count': envelope['completion_count'], 'branch_certificate_digests': envelope['branch_certificate_digests']}
...
def _text_projection_key(projection, *, budget) -> dict[str, object]:
    return {'source_cursor_digest': _cursor_envelope(projection.source_cursor, budget=budget, operation='snapshot_advance.text_projection_key.source_cursor')['digest'], 'emitted_text': projection.emitted_text, 'successor_cursor_digest': _cursor_envelope(projection.successor_cursor, budget=budget, operation='snapshot_advance.text_projection_key.successor_cursor')['digest'], 'immediate_multiplicity': projection.immediate_multiplicity}
