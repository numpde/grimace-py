...
def writer_snapshot_replay_envelope_for_emitted_texts(*, prepared: SouthStarPreparedMol, snapshot, emitted_texts: tuple[str, ...], budget: WriterEnvelopeWorkBudget | None=None) -> dict[str, object]:
    ...
    envelope = {'schema_name': SCHEMA_NAME, 'schema_version': SCHEMA_VERSION, 'prepared_identity': _identity_envelope(snapshot.prepared_identity, budget=budget, operation='envelope.identity'), 'source_snapshot': _snapshot_identity_envelope(snapshot, budget=budget, operation='envelope.identity'), 'emitted_texts': list(emitted_texts), 'outcome_kind': outcome.kind.value, 'consumed_emitted_texts': list(outcome.consumed_emitted_texts), 'remaining_emitted_texts': list(outcome.remaining_emitted_texts), 'step_advance_envelopes': list(advanced_step_envelopes), 'current_snapshot': _snapshot_identity_envelope(outcome.current_snapshot, budget=budget, operation='envelope.identity'), 'replay_certificate': None if outcome.replay_certificate is None else _replay_certificate_envelope(outcome.replay_certificate, budget=budget), 'failed_advance_envelope': failed_envelope}
...
def _replay_certificate_envelope(certificate, *, budget) -> dict[str, object]:
    return {'source_snapshot': _snapshot_identity_envelope(certificate.source_snapshot, budget=budget, operation='envelope.identity'), 'emitted_texts': list(certificate.emitted_texts), 'step_certificate_digests': [_identity_digest(step, budget=budget, operation='envelope.identity') for step in certificate.step_certificates], 'frontier_projection_digests': [_identity_digest(projection, budget=budget, operation='envelope.identity') for projection in certificate.frontier_projection_certificates], 'final_snapshot': _snapshot_identity_envelope(certificate.final_snapshot, budget=budget, operation='envelope.identity'), 'digest': _identity_digest(certificate, budget=budget, operation='envelope.identity')}
