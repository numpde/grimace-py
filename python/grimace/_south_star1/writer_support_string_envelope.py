...
def _text_projection_chain_envelope(certificate, *, budget):
    ...
    for index, step in enumerate(certificate.replay_certificate.step_certificates):
        ...
        chain.append({'step_index': index, 'emitted_text': step.emitted_text, 'source_cursor': _cursor_envelope(step.source_cursor, budget=budget, operation='envelope.identity'), 'successor_cursor': _cursor_envelope(step.successor_cursor, budget=budget, operation='envelope.identity'), 'text_projection': text_projection, 'frontier_projection': frontier_projection, 'step_certificate_digest': _identity_digest(step, budget=budget, operation='envelope.identity'), 'branch_certificate_identities': [_branch_certificate_identity_envelope(branch, budget=budget) for branch in step.branch_certificates]})
    return chain
