...
def _format_container(value: str) -> str:
    compose_file, service, rest = value.split(" ", 2)
    return f"`{compose_file}` `{service}` {rest}"
...
def _benchmark_environment(metadata: dict[str, Any]) -> BenchmarkEnvironment:
    ...
    return BenchmarkEnvironment(
        recorded_at_utc=str(metadata["recorded_at_utc"]),
        git_commit=str(metadata["git_commit"]),
        git_change=str(metadata["git_change"]),
        git_dirty=bool(metadata["git_dirty"]),
        platform=str(metadata["platform"]),
        python=str(metadata["python"]),
        rdkit=str(metadata["rdkit"]),
        cpu_model=machine.get("cpu_model"),
        visible_cpus=machine.get("visible_cpus"),
        cgroup_memory_limit_bytes=machine.get("cgroup_memory_limit_bytes"),
        container=ENUM_TIMING_CONTAINER,
    )


def _metadata_from_environment_row(row: dict[str, str]) -> dict[str, Any] | None:
    if not set(BenchmarkEnvironment.tsv_fieldnames()).issubset(row):
        return None
    return {
        "recorded_at_utc": row["recorded_at_utc"],
        "git_commit": row["git_commit"],
        "git_change": row["git_change"],
        "git_dirty": row["git_dirty"] == "True",
        "platform": row["platform"],
        "python": row["python"],
        "rdkit": row["rdkit"],
        "machine": {
            "cpu_model": row["cpu_model"] or None,
            "visible_cpus": int(row["visible_cpus"]) if row["visible_cpus"] else None,
            "cgroup_memory_limit_bytes": (
                int(row["cgroup_memory_limit_bytes"])
                if row["cgroup_memory_limit_bytes"]
                else None
            ),
        },
...
class EnumTimingBenchmark:
    OUTPUT_TSV_PATH = REPO_ROOT / "docs" / "timings-enum.tsv"
    OUTPUT_MD_PATH = REPO_ROOT / "docs" / "timings-enum.md"
    OUTPUT_PLOT_DIR = REPO_ROOT / "docs" / "timings-enum-plots"
    ...
    def _write_tsv(self, rows: list[TimingRow], metadata: dict[str, Any]) -> None:
        with self.OUTPUT_TSV_PATH.open("w", encoding="utf-8", newline="") as handle:
            ...
            writer.writeheader()
            environment_payload = asdict(_benchmark_environment(metadata))
            for row in rows:
    ...
    def _metadata_from_tsv(self) -> dict[str, Any] | None:
        ...
        if not rows:
            return None
        return _metadata_from_environment_row(rows[0])
    ...
    def _draw_timing_plot(ax, row: dict[str, str]) -> None:
        ...
        legend = EnumTimingBenchmark._add_timing_plot_legend(
            ax,
            bars,
            labels,
            means,
    ...
    def _add_timing_plot_legend(ax, bars, labels, means, stds):
        ...
        for preference, location in enumerate(("upper right", "lower right")):
            ...
            score = EnumTimingBenchmark._legend_bar_overlap(
                ax,
        ...
        append_history_record(
            {
                ...
                **metadata,
                "benchmark": "scripts.timings_enum_measure",
    ...
    def _render_document_from_tsv(
        ...
        for row in self._timing_rows_from_tsv():
            ...
            image_path = f"timings-enum-plots/{self._plot_filename(surface, surface_counts[surface])}"
    ...
    def _render_benchmark_environment(metadata: dict[str, Any]) -> tuple[str, ...]:
        environment = _benchmark_environment(metadata)
        rows = (
            ("Recorded", _format_recorded_at(environment.recorded_at_utc)),
            (
                "Runtime",
                f"Python {environment.python}, RDKit {environment.rdkit}",
            ),
            ("Platform", environment.platform),
            ("CPU", _format_cpu(asdict(environment))),
            (
                "Memory limit",
                _format_bytes(environment.cgroup_memory_limit_bytes),
            ),
            ("Container", _format_container(environment.container)),
        )
        lines = [
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Measure enum/support timings and render timing artifacts.",
    )
