...
class SouthStarFragmentCompositionTests(unittest.TestCase):
    ...
    def test_composition_deduplicates_symmetric_fragment_products(self) -> None:
        result = compose_disconnected_fragment_supports(
            (
                SouthStarFragmentSupport(fragment_id="left", outputs=("C",)),
                SouthStarFragmentSupport(fragment_id="right", outputs=("C",)),
            )
        )

        self.assertEqual(("C.C",), result.outputs)
        self.assertEqual(2, result.estimated_product_size)
        self.assertEqual(2, result.fragment_order_count)
        self.assertEqual((1, 1), result.fragment_output_counts)

    def test_composition_requires_at_least_one_fragment(self) -> None:
        with self.assertRaisesRegex(ValueError, "at least one fragment"):
