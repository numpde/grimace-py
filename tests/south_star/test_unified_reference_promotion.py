...
from tests.helpers.south_star_unified_reference import (
    ...
    two_atom_bond_text_support_proof_from_facts,
...
class SouthStarUnifiedReferencePromotionTests(unittest.TestCase):
    ...
    def test_two_atom_bond_text_cases_have_facts_derived_support(
        self,
    ) -> None:
        cases = {
            case.case_id: case for case in load_south_star_expanded_support_cases()
        }
        case = cases["quadruple_bond_text_carbon_carbon"]
        facts = SouthStarMoleculeFacts.from_mol(parse_smiles(case.source_smiles))

        self.assertTrue(is_two_atom_bond_text_domain(facts))
        proof = two_atom_bond_text_support_proof_from_facts(facts)
        self.assertEqual(case.expected_support, proof.support)
        self.assertEqual(
            "explicit_quadruple_bond",
            proof.bond_text_obligation.token_family,
        )
        result = mol_to_smiles_enum_s_graph_native(
            case.source_smiles,
            case_id=case.case_id,
