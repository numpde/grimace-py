...
class WriterStateKernelTest(unittest.TestCase):
    ...
    def _test_surviving_child_batch(
        self,
        child_surface: writer_transitions._WriterActiveChildScheduleSurface,
        *,
        emitted_text: str = "C",
    ) -> writer_transitions._WriterScheduledActionEmissionBatch:
        transition = SimpleNamespace(emitted_text=emitted_text)
        emissions = tuple(
            writer_transitions._WriterScheduledActionEmission(
                action=action,
        ...
        return writer_transitions._WriterScheduledActionEmissionBatch(
            actions=child_surface.scheduled_actions,
            emissions=emissions,
            surviving_emissions=emissions,
        )

    def _test_active_owned_dead_closure_active_outcome(
        self,
        *,
        closure_owner_kind: WriterBoundaryOwnerKind | None = (
            WriterBoundaryOwnerKind.ACTIVE_ATOM
        ...
        closure_decision, child_surface = (
            self._test_residual_cyclic_policy_inputs(
                ...
                child_attachment_action_kind=child_attachment_action_kind,
            )
        )
        child_batch = self._test_surviving_child_batch(
            child_surface,
            emitted_text=emitted_text,
        )
    ...
    def _test_top_level_active_owned_dead_closure_outcome(
        self,
        ...
        child_batch = self._test_surviving_child_batch(child_surface)
        ...
        with patch(
            ...
            return writer_transitions._top_level_schedule_outcome(
                prepared,  # type: ignore[arg-type]
                state,  # type: ignore[arg-type]
                context,  # type: ignore[arg-type]
            )
    ...
    def _test_prepared_active_owned_cycle_surfaces(
        self,
    ) -> tuple[
        object,
        WriterState,
        ...
        opened_states = tuple(
            ...
            for transition in writer_transitions.legal_writer_transitions(
                prepared,
                root_state,
            )
        )

        for opened_state in opened_states:
            opened_context = (
            ...
            child_surface = (
                writer_transitions._active_child_schedule_surface_from_context(
                    opened_context,
                    opened_state,
            ...
            if child_surface.considered_cyclic_tree_entry_available:
                return (
                    prepared,
                    root_state,
                    root_context,
    ...
    def _test_prepared_active_owned_cycle_frontier_fixture(
        ...
        *,
        ...
        for transition in writer_transitions.legal_writer_transitions(
            ...
            candidate_surface = (
                writer_transitions._active_child_schedule_surface_from_context(
                    candidate_context,
                    candidate_state,
                    candidate_state.active.atom,
                )
            )

            if candidate_surface == child_surface:
        ...
        closure_decision = self._test_closure_decision_with_open_emission_evidence(
            closure_decision,
            include_open_emissions=include_open_emissions,
        )
        ...
        return SimpleNamespace(
            ...
            state_key=writer_state_key(selected_state),
    ...
    def _test_prepared_active_owned_cycle_side_effects(self, fixture):
        original_closure = writer_transitions._closure_endpoint_schedule_decision
        original_child = (
            writer_transitions._active_child_schedule_surface_from_context
        )

        def closure_endpoint_schedule_decision(prepared, state, context):
            if writer_state_key(state) == fixture.state_key:
                return fixture.closure_decision

            return original_closure(prepared, state, context)

        def active_child_schedule_surface_from_context(context, state, active_atom):
            if writer_state_key(state) == fixture.state_key:
                return fixture.child_surface

            return original_child(context, state, active_atom)

        return (
            closure_endpoint_schedule_decision,
            active_child_schedule_surface_from_context,
        )

    def _test_closure_decision_with_open_emission_evidence(
        self,
        closure_decision: (
            writer_transitions._WriterClosureEndpointScheduleDecision
        ),
        *,
        include_open_emissions: bool,
    ) -> writer_transitions._WriterClosureEndpointScheduleDecision:
        open_emissions = (
            tuple(
                writer_transitions._WriterScheduledActionEmission(
            ...
            if include_open_emissions
            else ()
        )

        return writer_transitions._WriterClosureEndpointScheduleDecision(
            pair_batch=writer_transitions._WriterScheduledActionEmissionBatch(
                actions=closure_decision.schedule_surface.pair_actions,
                emissions=(),
                surviving_emissions=(),
            ),
            open_batch=writer_transitions._WriterScheduledActionEmissionBatch(
                actions=closure_decision.schedule_surface.open_actions,
                emissions=open_emissions,
                surviving_emissions=(),
            ),
            surviving_emissions=(),
            schedule_surface=closure_decision.schedule_surface,
        )
    ...
    def _test_frontier_choice_snapshot_for_active_outcome(
        ...
        *,
        ...
        support = self._test_frontier_next_token_support(
            emitted_text=emitted_text,
            successor_atom=AtomId(5),
            policy_family=support_family,
    ...
    def _test_writer_search_snapshot(
        self,
    ) -> writer_snapshot.WriterSearchSnapshot:
        cursor = WriterFrontierCursor(weighted_states=())

        return writer_snapshot.WriterSearchSnapshot(
            serialization_language=SerializationLanguageMode.WRITER_SHAPED,
            prepared_identity=object(),  # type: ignore[arg-type]
            runtime_options=object(),  # type: ignore[arg-type]
            cursor=cursor,
            decoder_boundary=writer_snapshot.WriterDecoderBoundary(),
            frame_stack=(writer_snapshot.WriterFrontierFrame(cursor),),
        )

    def _test_readable_prefix_outcome(
        self,
        choice_snapshot: writer_frontier_module._WriterFrontierChoiceSnapshot,
        *,
        replay_choice: (
            writer_frontier_module._WriterFrontierChoiceSnapshotEntry | None
        ) = None,
    ) -> writer_snapshot._WriterSnapshotPrefixReadOutcome:
        source_snapshot = self._test_writer_search_snapshot()
        ...
        if replay_choice is None:
            ...
            emitted_texts = (replay_choice.emitted_text,)
            advanced_snapshot = self._test_writer_search_snapshot()
            step_outcomes = (
                writer_snapshot._WriterSnapshotAdvanceOutcome(
                    kind=writer_snapshot._WriterSnapshotAdvanceOutcomeKind.ADVANCED,
                    source_snapshot=source_snapshot,
                    emitted_text=replay_choice.emitted_text,
                    choice_snapshot=choice_snapshot,
    ...
    def test_writer_frontier_residual_attachment_evidence_group_exposes_residual_cyclic_decisions(self) -> None:
        ...
        unsupported_closure, unsupported_child = (
            self._test_residual_cyclic_policy_inputs(
                closure_owner_kind=WriterBoundaryOwnerKind.PENDING_PARENT,
                child_owner_kind=WriterBoundaryOwnerKind.PENDING_PARENT,
            )
        )
    ...
    def test_active_emitted_graph_policy_selects_surviving_closure_without_children(self) -> None:
        ...
        self.assertIsNone(decision.residual_cyclic_policy_decision)
        child_surface.assert_not_called()
        residual_policy.assert_not_called()
    ...
    def test_active_owned_residual_cyclic_policy_decision_classifies_support_dead_resolution(self) -> None:
        closure_decision, child_surface = self._test_residual_cyclic_policy_inputs()
        choice_groups = (
            writer_transitions
            ._closure_open_vs_cyclic_tree_entry_policy_groups(
    ...
    def test_active_owned_residual_cyclic_policy_decision_rejects_unsupported_owner_scope(self) -> None:
        closure_decision, child_surface = (
            self._test_residual_cyclic_policy_inputs(
                closure_owner_kind=WriterBoundaryOwnerKind.BRANCH_RETURN,
                child_owner_kind=WriterBoundaryOwnerKind.BRANCH_RETURN,
    ...
    def test_branch_return_residual_cyclic_policy_decision_classifies_support_dead_resolution(self) -> None:
        closure_decision, child_surface = (
            self._test_residual_cyclic_policy_inputs(
                closure_owner_kind=WriterBoundaryOwnerKind.BRANCH_RETURN,
                child_owner_kind=WriterBoundaryOwnerKind.BRANCH_RETURN,
    ...
    def test_branch_return_residual_cyclic_policy_decision_classifies_missing_evidence(self) -> None:
        closure_decision, child_surface = (
            self._test_residual_cyclic_policy_inputs(
                closure_owner_kind=WriterBoundaryOwnerKind.BRANCH_RETURN,
                child_owner_kind=WriterBoundaryOwnerKind.BRANCH_RETURN,
    ...
    def test_residual_cyclic_policy_dispatcher_classifies_unsupported_owner_scope_before_active_owned_rule(self) -> None:
        closure_decision, child_surface = (
            self._test_residual_cyclic_policy_inputs(
                closure_owner_kind=WriterBoundaryOwnerKind.PENDING_PARENT,
                child_owner_kind=WriterBoundaryOwnerKind.PENDING_PARENT,
    ...
    def test_residual_cyclic_policy_dispatcher_delegates_branch_return_groups(self) -> None:
        ...
        returned_decision = writer_transitions._WriterResidualCyclicPolicyDecision(
            kind=(
                writer_transitions
                ._WriterResidualCyclicPolicyDecisionKind
                .BRANCH_RETURN_AFTER_DEAD_CLOSURE_OPEN
            ...
            support_dead_groups=choice_groups,
        )
    ...
    def test_residual_cyclic_policy_decision_maps_to_active_emitted_policy_kind(self) -> None:
        ...
        branch_closure_decision, branch_child_surface = (
            self._test_residual_cyclic_policy_inputs(
                closure_owner_kind=WriterBoundaryOwnerKind.BRANCH_RETURN,
                child_owner_kind=WriterBoundaryOwnerKind.BRANCH_RETURN,
            )
        ...
        cases = (
            ...
            (
                writer_transitions._WriterResidualCyclicPolicyDecision(
                    ...
                    closure_endpoint_decision=branch_closure_decision,
                    child_schedule_surface=branch_child_surface,
                    choice_groups=(group,),
                    support_dead_groups=(group,),
                ),
                (
        ...
        for (
            ...
            with self.subTest(kind=decision.kind):
                ...
                self.assertIs(
                    decision.selects_plain_active_child,
                    selects_plain_child,
                )
                self.assertIs(decision.selects_active_child, selects_child)
                self.assertIs(
                    decision.blocks_active_emitted_policy,
                    blocks_policy,
                )
                self.assertIs(decision.blocks_active_child, blocks_policy)

    def test_residual_cyclic_policy_decision_matches_active_emitted_policy_kind(self) -> None:
        active_atom = AtomId(0)
        key = writer_transitions._WriterResidualAttachmentPolicyKey(
            active_atom,
            11,
        ...
        graph_policy_kinds = tuple(
            writer_transitions._WriterActiveEmittedGraphPolicyDecisionKind
        )

        for decision, expected_kind, requires_blocker in cases:
            with self.subTest(kind=decision.kind):
                for graph_policy_kind in graph_policy_kinds:
                    self.assertIs(
                        decision.maps_to_active_emitted_graph_policy_kind(
                            graph_policy_kind
                        ),
                        graph_policy_kind is expected_kind,
                    )

                self.assertIs(
                    decision.requires_active_emitted_blocker,
                    requires_blocker,
                )
    ...
    def test_active_emitted_graph_policy_maps_residual_cyclic_policy_decision(self) -> None:
        ...
        cases = (
            ...
            (
                ...
                branch_closure_decision,
                branch_child_surface,
                (
                    writer_transitions
                    ._WriterActiveEmittedGraphPolicyDecisionKind
                    .ACTIVE_CHILD_AFTER_DEAD_CLOSURE_OPEN
        ...
        for residual_decision, closure_decision, child_surface, expected_kind in cases:
            with self.subTest(kind=residual_decision.kind):
                self.assertIs(
                    residual_decision.active_emitted_graph_policy_kind,
                    expected_kind,
                )
                with patch(
                    "grimace._south_star1.writer_transitions._closure_endpoint_schedule_decision",
    ...
    def test_active_emitted_graph_policy_accepts_matching_retained_residual_cyclic_decision(self) -> None:
        ...
        for closure_decision, child_surface, graph_policy_kind in cases:
            ...
            with self.subTest(kind=residual_decision.kind):
                ...
                self.assertTrue(
                    residual_decision
                    .maps_to_active_emitted_graph_policy_kind(
                        graph_policy.kind
                    )
                )
                self.assertIs(
    ...
    def test_active_emitted_graph_policy_rejects_mismatched_retained_residual_cyclic_decision(self) -> None:
        ...
        blocked_child_surface = writer_transitions._WriterActiveChildScheduleSurface(
            active_atom=AtomId(0),
            blockers=(
                writer_transitions._WriterChildObligationBlocker(
                    kind=(
                        writer_transitions
        ...
        invalid_payloads = (
            ...
            dict(
                ...
                closure_endpoint_decision=unsupported_closure_decision,
                child_schedule_surface=unsupported_child_surface,
                residual_cyclic_policy_decision=unsupported_decision,
            ),
            dict(
                kind=(
                    writer_transitions
                ...
                closure_endpoint_decision=missing_closure_decision,
                child_schedule_surface=missing_child_surface,
                residual_cyclic_policy_decision=missing_decision,
            ),
            dict(
                kind=(
                    writer_transitions
            ...
            dict(
                ...
                closure_endpoint_decision=none_closure_decision,
                child_schedule_surface=blocked_child_surface,
                residual_cyclic_policy_decision=none_decision,
            ),
        )
    ...
    def test_active_emitted_graph_policy_allows_branch_return_dead_closure_open_support(self) -> None:
        prepared = object()
        state = object()
        context = object()
        active_atom = AtomId(0)
        ...
        self.assertIs(
            decision.residual_cyclic_policy_kind,
            (
                writer_transitions
                ._WriterResidualCyclicPolicyDecisionKind
                .BRANCH_RETURN_AFTER_DEAD_CLOSURE_OPEN
        ...
        self.assertTrue(decision.emits_child_actions)
        self.assertTrue(decision.resolved_residual_attachment_policy_groups)
        self.assertEqual(decision.graph_policy_blockers, ())
        self.assertTrue(
            (
                decision
    ...
    def test_active_owned_dead_closure_policy_reaches_active_emitted_schedule_outcome(self) -> None:
        ...
        self.assertIs(
            outcome.graph_policy_decision.kind,
            (
                writer_transitions
                ._WriterActiveEmittedGraphPolicyDecisionKind
                .ACTIVE_CHILD_AFTER_DEAD_CLOSURE_OPEN
        ...
        self.assertIs(
            outcome.graph_policy_decision.residual_cyclic_policy_kind,
            (
                writer_transitions
                ._WriterResidualCyclicPolicyDecisionKind
                .ACTIVE_CHILD_AFTER_DEAD_CLOSURE_OPEN
        ...
        self.assertTrue(
            outcome.graph_policy_decision.residual_cyclic_support_dead_groups
        )
        self.assertIs(
            outcome.schedule_decision.kind,
            writer_transitions._WriterActiveEmittedScheduleDecisionKind.ACTIVE_CHILD,
        ...
        self.assertIs(
            outcome.schedule_decision.selected_active_child_selection_kind,
            writer_transitions._WriterActiveChildSelectionKind.CYCLIC_TREE_ENTRY,
        )

    def test_active_emitted_schedule_outcome_emits_child_for_branch_return_dead_closure_open_support(self) -> None:
        outcome = self._test_active_owned_dead_closure_active_outcome(
            closure_owner_kind=WriterBoundaryOwnerKind.BRANCH_RETURN,
            child_owner_kind=WriterBoundaryOwnerKind.BRANCH_RETURN,
        )
    ...
    def test_active_owned_dead_closure_policy_reaches_top_level_schedule_outcome(self) -> None:
        outcome = self._test_top_level_active_owned_dead_closure_outcome()
        ...
        self.assertIs(
            outcome.selected_active_child_selection_kind,
            writer_transitions._WriterActiveChildSelectionKind.CYCLIC_TREE_ENTRY,
        )
        self.assertTrue(outcome.selected_cyclic_tree_entry_graph_action_surfaces)
        self.assertEqual(outcome.graph_policy_blockers, ())

    def test_active_owned_dead_closure_policy_reaches_frontier_choice_snapshot_evidence(self) -> None:
        outcome = self._test_active_owned_dead_closure_active_outcome()
    ...
    def test_active_owned_dead_closure_policy_is_visible_in_prefix_read_evidence(self) -> None:
        outcome = self._test_active_owned_dead_closure_active_outcome()
        residual = outcome.graph_policy_decision.residual_cyclic_policy_decision
        self.assertIsNotNone(residual)
        key = residual.choice_groups[0].key  # type: ignore[union-attr]
        ...
        final_prefix = self._test_readable_prefix_outcome(choice_snapshot)
        ...
        self.assertIn(
            ...
            final_prefix.residual_cyclic_policy_kinds,
        )
        self.assertTrue(
            (
                final_prefix
        ...
        self.assertEqual(final_prefix.replayed_residual_cyclic_policy_kinds, ())
        self.assertEqual(
            (
                final_prefix
                .replayed_dead_closure_open_resolved_cyclic_tree_entry_evidence
        ...
        replayed_prefix = self._test_readable_prefix_outcome(
            choice_snapshot,
            replay_choice=choice_snapshot.choices[0],
        )
        self.assertIn(
            (
                writer_transitions
            ...
            replayed_prefix.replayed_residual_cyclic_policy_kinds,
        )
        self.assertTrue(
            (
                replayed_prefix
    ...
    def test_branch_return_dead_closure_open_support_reaches_prefix_read_evidence(self) -> None:
        outcome = self._test_active_owned_dead_closure_active_outcome(
            closure_owner_kind=WriterBoundaryOwnerKind.BRANCH_RETURN,
            child_owner_kind=WriterBoundaryOwnerKind.BRANCH_RETURN,
        )
    ...
    def test_active_owned_dead_closure_evidence_does_not_mark_acyclic_child_as_cyclic(self) -> None:
        outcome = self._test_active_owned_dead_closure_active_outcome(
            child_attachment_action_kind=(
                WriterResidualAttachmentActionKind.ACYCLIC_TREE_ENTRY
            ),
    ...
    def test_prepared_active_owned_cycle_feeds_closure_open_vs_cyclic_tree_policy_groups(self) -> None:
        (
            _prepared,
            _state,
            _context,
    ...
    def test_prepared_active_owned_cycle_missing_closure_open_evidence_stays_unresolved(self) -> None:
        ...
        missing_closure = self._test_closure_decision_with_open_emission_evidence(
            closure_decision,
    ...
    def test_prepared_non_active_owned_cycle_remains_unsupported_owner_scope(self) -> None:
        closure_decision, child_surface = (
            self._test_residual_cyclic_policy_inputs(
                closure_owner_kind=WriterBoundaryOwnerKind.PENDING_PARENT,
                child_owner_kind=WriterBoundaryOwnerKind.PENDING_PARENT,
        ...
        self.assertTrue(policy.unsupported_owner_scope_kinds)
        self.assertTrue(policy.graph_policy_blockers)

    def test_prepared_acyclic_child_with_dead_closure_open_does_not_trigger_residual_cyclic_policy(self) -> None:
        closure_decision = self._test_prepared_active_owned_cycle_surfaces()[3]
        support_dead_closure = (
            self._test_closure_decision_with_open_emission_evidence(
                closure_decision,
                include_open_emissions=True,
        ...
        policy = writer_transitions._WriterActiveEmittedGraphPolicyDecision(
            kind=writer_transitions._WriterActiveEmittedGraphPolicyDecisionKind.ACTIVE_CHILD,
            active_atom=AtomId(0),
            closure_endpoint_decision=support_dead_closure,
            child_schedule_surface=child_surface,
            residual_cyclic_policy_decision=(
                writer_transitions._residual_cyclic_policy_decision(
                    support_dead_closure,
    ...
    def test_prepared_active_owned_cyclic_policy_reaches_frontier_choice_snapshot(self) -> None:
